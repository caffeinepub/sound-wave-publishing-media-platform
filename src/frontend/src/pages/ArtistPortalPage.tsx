...
import { useGetArtistPortalProfile } from "../hooks/useQueries";
            ...
            <div className="space-y-2">
              ...
              {!isEligible && (
                <p className="text-xs text-muted-foreground">
                  {REQUIRED_PIECES - submittedPieces} more piece
                  {REQUIRED_PIECES - submittedPieces !== 1 ? "s" : ""} needed
                  for eligibility
                </p>
              )}
              {isEligible && (
          ...
          <CardHeader>
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <CardDescription className="mt-1">
                  Sound Waves Publishing &amp; Media — 10,000,000 total shares
                  at $1.00/share
                </CardDescription>
              </div>
              <Link to="/shares/certificates">
                <Button
                  variant="outline"
                  size="sm"
          ...
          <CardContent className="space-y-4">
            <div className="grid gap-4 sm:grid-cols-3">
              ...
              <div className="rounded-lg border border-border bg-muted/30 p-4 text-center space-y-1">
                <p className="text-2xl font-bold font-display">
                  ${(portalProfile?.totalPaid ?? 0).toFixed(2)}
                </p>
                <p className="text-xs text-muted-foreground uppercase tracking-wide">
                  Total Invested
                </p>
            ...
            {isEligible && qualifiesForFreeShare && !freeShareClaimed && (
              <Alert className="border-primary/30 bg-primary/10">
                ...
                <AlertDescription className="text-primary">
                  <strong>Congratulations!</strong> You qualify for 1 free share
                  as one of the first 100 eligible artists. This will be
                  credited automatically when you complete your first share
                  purchase.
                </AlertDescription>
              </Alert>
            )}
            ...
            {!isEligible && (
              <Alert className="border-border">
                ...
                <AlertDescription className="text-muted-foreground">
                  Submit {REQUIRED_PIECES - submittedPieces} more musical piece
                  {REQUIRED_PIECES - submittedPieces !== 1 ? "s" : ""} to unlock
                  share purchases.
                </AlertDescription>
              </Alert>
            )}
            ...
            {isEligible && maxPurchasable === 0 && (
              <Alert className="border-primary/30 bg-primary/10">
                ...
                <AlertDescription className="text-primary">
                  You've reached the maximum of {MAX_SHARES} shares. Thank you
                  for your investment in Sound Waves Publishing &amp; Media!
                </AlertDescription>
              </Alert>
            )}
            ...
            {isEligible && maxPurchasable > 0 && (
              <SharePurchaseForm
                ...
                qualifiesForFreeShare={
                  qualifiesForFreeShare && !freeShareClaimed
                }
