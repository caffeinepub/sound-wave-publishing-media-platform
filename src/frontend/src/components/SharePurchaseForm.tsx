...
import { CreditCard, Gift, Loader2, TrendingUp } from "lucide-react";
...
export default function SharePurchaseForm({
  ...
  const paidQuantity = qualifiesForFreeShare
    ? Math.max(0, effectiveQuantity - 1)
    : effectiveQuantity;
  ...
  const handlePurchase = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(
        err.message || "Failed to initiate share purchase. Please try again.",
      );
    }
  };
        ...
        <Input
          ...
          onChange={(e) =>
            setQuantity(
              Math.max(
                1,
                Math.min(maxPurchasable, Number.parseInt(e.target.value) || 1),
              ),
            )
          }
      ...
      >
        {purchaseShares.isPending ? (
          ...
          <>
            ...
            Purchase {effectiveQuantity} Share
            {effectiveQuantity !== 1 ? "s" : ""} — ${totalDollars}
          </>
        )}
      </Button>
    </form>
