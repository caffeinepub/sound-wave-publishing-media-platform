...
import {
  useCreateLightscribeProject,
  useUpdateLightscribeProject,
...
<lightscribe version="1.0">
  <label>
    ...
    const lsaBlob = new TextEncoder().encode(
      lsaContent,
...
`;
    ...
    const lssBlob = new TextEncoder().encode(
      lssContent,
    ) as Uint8Array<ArrayBuffer>;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const layerData = JSON.stringify({
        layers: elements.map((el, i) => ({ id: el.id, zIndex: i })),
      });
      ...
      const project: LightscribeLabelProject = {
        ...
        metadata,
        createdTimestamp:
          existingProject?.createdTimestamp || BigInt(Date.now() * 1000000),
  ...
  const handleExport = async (format: "lsa" | "lss" | "png") => {
    try {
      if (format === "png") {
        ...
        const { lsaBlob, lssBlob } = await generateLightscribeFiles();
        const blob = new Blob([format === "lsa" ? lsaBlob : lssBlob], {
          type: "text/plain",
        });
        const url = URL.createObjectURL(blob);
        const a = document.createElement("a");
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            {existingProject ? "Edit" : "Create"} CD Label Project
          </DialogTitle>
        </DialogHeader>
