...
import {
  useCreateTrademarkRecord,
  useUpdateTrademarkRecord,
...
export default function CreateTrademarkDialog({
  artistId,
  existingTrademark,
  trigger,
}: CreateTrademarkDialogProps) {
  const [open, setOpen] = useState(false);
  const [markName, setMarkName] = useState(existingTrademark?.markName || "");
  const [registrationNumber, setRegistrationNumber] = useState(
    existingTrademark?.registrationNumber || "",
  );
  const [status, setStatus] = useState(existingTrademark?.status || "Pending");
  const [description, setDescription] = useState(
    existingTrademark?.description || "",
  );
  const [file, setFile] = useState<File | null>(null);
  const [uploadProgress, setUploadProgress] = useState(0);
  const [filingDate, setFilingDate] = useState(
    existingTrademark
      ? new Date(Number(existingTrademark.filingDate) / 1000000)
          .toISOString()
          .split("T")[0]
      : "",
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (file) {
        ...
        documentReference = ExternalBlob.fromBytes(
          fileBytes,
      ...
      const trademarkId =
        existingTrademark?.id ||
        `tm-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
      ...
      toast.error(
        error.message ||
          `Failed to ${existingTrademark ? "update" : "create"} trademark`,
      );
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
        <DialogHeader>
          <DialogTitle>
            {existingTrademark ? "Edit Trademark" : "Add New Trademark"}
          </DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-2">
              ...
              {existingTrademark?.documentReference && !file && (
                <Button
                  ...
                  onClick={() =>
                    window.open(
                      existingTrademark.documentReference!.getDirectURL(),
                      "_blank",
                    )
                  }
                >
            ...
            <p className="text-xs text-muted-foreground">
              Upload trademark registration documents or certificates
            </p>
          </div>
