...
import { useGetMedia, useCreateUnifiedCheckoutSession, useRecordEvent } from '../hooks/useQueries';
...
import type { ShoppingItem, CheckoutRequest, LicensingOption } from '../backend';
...
export default function CheckoutHandoffPage() {
  ...
  const paymentMethodParam = (searchParams as any).paymentMethod as string | undefined;

  // Fetch media data
  const { data: media, isLoading: mediaLoading, error: mediaError } = useGetMedia(mediaId);
  ...
  useEffect(() => {
    if (isAuthenticated && !hasAttemptedCheckout && !isProcessing && media && licenseId) {
      initiateCheckout();
    }
  }, [isAuthenticated, hasAttemptedCheckout, media, licenseId]);
  ...
  const initiateCheckout = async () => {
    ...
    try {
      ...
      const license = media.licensingOptions.find((opt: LicensingOption) => opt.id === licenseId);
        ...
        <Card className="w-full max-w-md">
          ...
          <CardContent className="space-y-4 text-center">
            <p className="text-muted-foreground">
              The checkout link is missing required information. Please use a valid checkout link or browse the gallery to select a work.
            </p>
    ...
    return (
      <div className="container flex min-h-[600px] items-center justify-center py-16">
        <Card className="w-full max-w-md">
          ...
          <CardContent className="space-y-4 text-center">
            <p className="text-muted-foreground">
              The work you're trying to purchase could not be found. It may have been removed or the link is invalid.
            </p>
            <Button onClick={handleBackToGallery} className="w-full">
              Back to Gallery
            </Button>
    ...
    return (
      <div className="container flex min-h-[600px] items-center justify-center py-16">
        <Card className="w-full max-w-md">
          ...
          <CardContent className="space-y-4 text-center">
            <p className="text-muted-foreground">
              You need to be logged in to purchase a license. Please login to continue with your purchase.
            </p>
            <div className="rounded-lg border bg-muted/50 p-4">
              <p className="text-sm font-medium">{media.title}</p>
              <p className="text-sm text-muted-foreground">{license.name}</p>
              <p className="mt-2 text-lg font-bold">${(Number(license.price) / 100).toFixed(2)}</p>
            </div>
