...
import { Music, DollarSign, Package, Trash2, FileText, Edit, Shield, Disc, Plus, Download, Settings } from 'lucide-react';
...
export default function ArtistDashboard() {
  ...
  const [deleteTrademarkId, setDeleteTrademarkId] = useState<string | null>(null);
  ...
  }, [identity, userProfile, allArtists, navigate, artistWorks]);
  ...
  const totalRevenue = sales.reduce((sum, sale) => {
    ...
    if (!media) return sum;
    const license = media.licensingOptions.find((l) => l.id === sale.licensingOptionId);
        ...
        <Card className="mx-auto max-w-2xl">
          ...
          <CardContent>
            <p className="mb-6 text-muted-foreground">
              You need to create an artist profile before you can upload and manage your creative works.
            </p>
            <Button onClick={() => setShowCreateProfile(true)}>Create Artist Profile</Button>
          </CardContent>
        </Card>
        <CreateArtistProfileDialog open={showCreateProfile} onOpenChange={setShowCreateProfile} />
      </div>
    );
  }
    ...
    <div className="container py-8">
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-muted-foreground">Manage your creative works and track sales</p>
        </div>
      ...
      <div className="mb-8 grid gap-4 md:grid-cols-5">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{lightscribeProjects.length}</div>
          </CardContent>
        </Card>
      </div>
            ...
            <Card>
              <CardContent className="flex min-h-[300px] items-center justify-center">
                <div className="text-center">
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">Upload your first creative work to get started</p>
                </div>
              </CardContent>
            </Card>
          ) : (
                  ...
                  {sales.map((sale) => {
                    ...
                    return (
                      <div key={sale.id} className="flex items-center justify-between p-4">
                        <div>
                          <p className="font-medium">{media?.title || 'Unknown Media'}</p>
                          <p className="text-sm text-muted-foreground">{license?.name || 'Unknown License'}</p>
                          <p className="text-xs text-muted-foreground">
                            {new Date(Number(sale.timestamp) / 1000000).toLocaleDateString()}
                          </p>
                        </div>
                        <Badge variant="secondary">${(Number(license?.price || 0) / 100).toFixed(2)}</Badge>
                      </div>
                    );
                  })}
                </div>
              ...
              <CardContent className="flex min-h-[300px] items-center justify-center">
                <div className="text-center">
                  ...
                  <p className="text-lg font-medium">
                    {trademarks.length === 0 ? 'No trademarks yet' : 'No trademarks match your filters'}
                  </p>
                  <p className="mt-1 text-sm text-muted-foreground">
                    {trademarks.length === 0
                ...
                <Card key={trademark.id} className="relative">
                  <CardHeader>
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <CardTitle className="text-lg">{trademark.markName}</CardTitle>
                        <p className="mt-1 text-sm text-muted-foreground">Reg. #{trademark.registrationNumber}</p>
                      </div>
                      <Badge className={getStatusColor(trademark.status)}>{trademark.status}</Badge>
                    </div>
                  </CardHeader>
                  <CardContent className="space-y-3">
                    <div>
                      ...
                      <p className="text-sm">
                        {new Date(Number(trademark.filingDate) / 1000000).toLocaleDateString()}
                      </p>
                    </div>
                    {trademark.description && (
                      <div>
                        <p className="text-xs font-medium text-muted-foreground">Description</p>
                        <p className="text-sm line-clamp-2">{trademark.description}</p>
                      </div>
                    )}
                    <div className="flex gap-2 pt-2">
                      {trademark.documentReference && (
                        <Button
                          ...
                          size="sm"
                          className="flex-1"
                          onClick={() => window.open(trademark.documentReference!.getDirectURL(), '_blank')}
                        >
        ...
        <TabsContent value="lightscribe" className="space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <img src="/assets/generated/lightscribe-icon-transparent.dim_64x64.png" alt="Lightscribe" className="h-8 w-8" />
            ...
            <Card>
              <CardContent className="flex min-h-[300px] items-center justify-center">
                <div className="text-center">
                  ...
                  <p className="text-lg font-medium">No CD label projects yet</p>
                  <p className="mt-1 text-sm text-muted-foreground">
                ...
                <Card key={project.id} className="overflow-hidden">
                  ...
                  <CardContent className="space-y-3">
                    <div className="flex gap-2 text-xs text-muted-foreground">
                      <span>Created: {new Date(Number(project.createdTimestamp) / 1000000).toLocaleDateString()}</span>
                      <span>•</span>
                      <span>Modified: {new Date(Number(project.modifiedTimestamp) / 1000000).toLocaleDateString()}</span>
                    </div>
                    <div className="flex gap-2">
      ...
      <AlertDialog open={!!deleteMediaId} onOpenChange={() => setDeleteMediaId(null)}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={handleDeleteMedia} disabled={deleteMedia.isPending}>
      ...
      <AlertDialog open={!!deleteTrademarkId} onOpenChange={() => setDeleteTrademarkId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete Trademark</AlertDialogTitle>
            <AlertDialogDescription>
              Are you sure you want to delete this trademark record? This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={handleDeleteTrademark} disabled={deleteTrademark.isPending}>
      ...
      <AlertDialog open={!!deleteProjectId} onOpenChange={() => setDeleteProjectId(null)}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={handleDeleteProject} disabled={deleteProject.isPending}>
