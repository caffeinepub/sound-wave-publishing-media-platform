...
export default function AdminAnalyticsPage() {
    ...
    return (
      <div className="container py-16">
        <Alert variant="destructive">
          ...
          <AlertDescription>
            Failed to load analytics data: {analyticsError instanceof Error ? analyticsError.message : 'Unknown error'}. Please try again later.
          </AlertDescription>
        </Alert>
  ...
  const elasticStageData = analyticsData.refCounts.find((ref) => ref.ref === 'elasticstage');
    ...
    <div className="container py-8">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 mb-8">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{Number(analyticsData.totalEvents).toLocaleString()}</div>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">ElasticStage Referrals</CardTitle>
            <Badge variant="secondary">elasticstage</Badge>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">
              {elasticStageData ? Number(elasticStageData.total).toLocaleString() : '0'}
            </div>
            <p className="text-xs text-muted-foreground mt-1">Events from ElasticStage</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{analyticsData.refCounts.length}</div>
            <p className="text-xs text-muted-foreground mt-1">Unique referral sources</p>
          </CardContent>
        </Card>
      </div>
              ...
              <TableBody>
                {elasticStageData.events.map((event) => (
                  <TableRow key={event.eventType}>
                    ...
                    <TableCell className="text-right">{Number(event.count).toLocaleString()}</TableCell>
                  </TableRow>
                ))}
                <TableRow className="bg-muted/50 font-semibold">
                  <TableCell>Total</TableCell>
                  <TableCell className="text-right">{Number(elasticStageData.total).toLocaleString()}</TableCell>
                </TableRow>
              </TableBody>
            </Table>
          </CardContent>
                  ...
                  <TableRow key={refData.ref || 'unknown'}>
                    <TableCell>
                      <div className="flex items-center gap-2">
                        {refData.ref === 'elasticstage' && <Badge variant="secondary">elasticstage</Badge>}
                        {refData.ref !== 'elasticstage' && (
                          <span className="font-medium">{refData.ref || 'Direct'}</span>
                        )}
                      </div>
                    </TableCell>
                    <TableCell className="text-right">{Number(refData.total).toLocaleString()}</TableCell>
                  </TableRow>
                ))}
              {allData && (
                <TableRow className="bg-muted/50 font-semibold">
                  <TableCell>Overall Total (including missing ref)</TableCell>
                  <TableCell className="text-right">{Number(allData.total).toLocaleString()}</TableCell>
                </TableRow>
              )}
            </TableBody>
          </Table>
      ...
      <div className="mt-6 text-center text-sm text-muted-foreground">
        Last updated: {new Date(Number(analyticsData.lastUpdated) / 1000000).toLocaleString()}
      </div>
    </div>
  );
}
