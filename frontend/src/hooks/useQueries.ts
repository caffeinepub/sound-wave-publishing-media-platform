...
export function useUpdateArtistProfile() {
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ['artistProfiles'] });
      queryClient.invalidateQueries({ queryKey: ['artistProfile', variables.id] });
    },
  });
}
...
export function useSetMerchantServicesConfiguration() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['merchantServicesConfigured'] });
    },
  });
}
...
export function useSetNationalBankcardConfiguration() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['nationalBankcardConfigured'] });
    },
  });
}
...
export function useCreateUnifiedCheckoutSession() {
  ...
  return useMutation({
    mutationFn: async (request: CheckoutRequest): Promise<CheckoutSessionResponse> => {
      if (!actor) throw new Error('Actor not available');
...
export function useUpdateLightscribeProject() {
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ['myLightscribeProjects'] });
      queryClient.invalidateQueries({ queryKey: ['lightscribeProject', variables.id] });
    },
  });
}
...
export function useRecordEvent() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      if (!actor) return;
      await actor.recordEvent({ caller: Principal.anonymous() }, eventType, ref ?? null, mediaId ?? null);
    },
  });
}
...
export function useCreateMembershipFeeRecord() {
  ...
  return useMutation({
    mutationFn: async ({ artistId, amount, notes }: { artistId: string; amount: bigint; notes?: string }) => {
      if (!actor) throw new Error('Actor not available');
...
export function useCreateInvoiceRecord() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      if (!actor) throw new Error('Actor not available');
      return actor.createInvoiceRecord(artistId, amount, description, mediaId ?? null, notes ?? null);
    },
    onSuccess: () => {
...
export function useCreatePayoutRecord() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      if (!actor) throw new Error('Actor not available');
      return actor.createPayoutRecord(artistId, amount, destinationAccount, payoutMethod, associatedMediaId ?? null, notes ?? null);
    },
    onSuccess: () => {
...
export function useAssignUserRole() {
  ...
  return useMutation({
    mutationFn: async ({ userId, role }: { userId: Principal; role: string }) => {
      if (!actor) throw new Error('Actor not available');
...
function getFreeShareCounter(): number {
  return parseInt(localStorage.getItem(FREE_SHARE_COUNTER_KEY) ?? '0', 10);
}
...
export function useGetArtistPortalProfile() {
  ...
  return useQuery<ArtistPortalProfile | null>({
    ...
    queryFn: async () => {
      ...
      if (data.isEligible && counter < FREE_SHARE_LIMIT && !data.freeShareClaimed) {
...
export function useSubmitMusicalPiece() {
  ...
  return useMutation({
    ...
    onSuccess: (_, _vars, _ctx) => {
      const principalId = identity?.getPrincipal().toString();
      queryClient.invalidateQueries({ queryKey: ['artistPortalProfile', principalId] });
    },
  });
}
...
export function useCreateEventRegistration() {
  ...
  return useMutation({
    mutationFn: async (input: EventRegistrationInput): Promise<CheckoutSession> => {
      if (!actor) throw new Error('Actor not available');
      ...
      const result = await actor.createCheckoutSession(items, input.successUrl, input.cancelUrl);
      const session = JSON.parse(result) as CheckoutSession;
...
export function usePurchaseShares() {
  ...
  return useMutation({
    mutationFn: async (input: SharePurchaseInput): Promise<CheckoutSession | null> => {
      if (!identity) throw new Error('Not authenticated');
      const principalId = identity.getPrincipal().toString();
      const data = loadPortalData(principalId);

      // Determine how many shares need payment (free share deducted)
      const qualifiesForFreeShare = data.isEligible && getFreeShareCounter() < FREE_SHARE_LIMIT && !data.freeShareClaimed;
      const paidQuantity = qualifiesForFreeShare ? Math.max(0, input.quantity - 1) : input.quantity;
...
function getAllPortalProfiles(): Array<{ principalId: string; data: ArtistPortalProfile }> {
  const results: Array<{ principalId: string; data: ArtistPortalProfile }> = [];
  for (let i = 0; i < localStorage.length; i++) {
    const key = localStorage.key(i);
    if (key && key.startsWith(PORTAL_STORAGE_PREFIX)) {
      const principalId = key.replace(PORTAL_STORAGE_PREFIX, '');
      try {
        const raw = localStorage.getItem(key);
        if (raw) results.push({ principalId, data: JSON.parse(raw) as ArtistPortalProfile });
      } catch {
        // ignore
      }
    }
