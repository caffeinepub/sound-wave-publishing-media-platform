...
export default function CreateArtistProfileDialog({ open, onOpenChange }: CreateArtistProfileDialogProps) {
  ...
  const validateElasticStageUrl = (url: string): boolean => {
    ...
    try {
      ...
      return parsed.protocol === 'https:' && parsed.hostname === 'elasticstage.com';
    } catch {
      return false;
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (elasticStageUrl.trim() && !validateElasticStageUrl(elasticStageUrl)) {
      toast.error('ElasticStage URL must be a valid https://elasticstage.com URL');
      return;
    }

    try {
      const profileId = `artist-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="elasticStageUrl" className="flex items-center gap-2">
            ...
            <p className="text-xs text-muted-foreground">
              External link to your ElasticStage profile (third-party site). Must be a valid https://elasticstage.com URL.
            </p>
          </div>
          <div className="space-y-2">
            <Label htmlFor="elasticStageId">ElasticStage Artist ID (optional)</Label>
            <Input
              id="elasticStageId"
