...
export default function AdminRegistrationsTab() {
  const { data: registrations = [], isLoading, error } = useGetAllEventRegistrations();

  const handleExportCSV = () => {
    const headers = ['Attendee Name', 'Email', 'Tickets', 'Total Paid ($)', 'Status', 'Date'];
    ...
    downloadCSV(csv, `event-registrations-${new Date().toISOString().split('T')[0]}.csv`);
  };
              ...
              <TableBody>
                {registrations.map((r, i) => (
                  <TableRow key={i} className="border-border">
                    ...
                    <TableCell className="text-right">{r.ticketQuantity}</TableCell>
                    <TableCell className="text-right text-primary font-semibold">${r.totalPaid.toFixed(2)}</TableCell>
                    <TableCell>
                      <Badge
                        variant="outline"
                        className={
